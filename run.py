...
for item_index in item_indexes:
  text_features = np.load(os.path.join(text_features_npy_dir, text_data.loc[item_index].file_name))
  ...
  image_features = np.load(os.path.join(image_features_npy_dir, image_data.loc[item_index].file_name))
...
seaborn.violinplot(data = meta_data[ ['text_min','text_max', 'image_max'] ], inner = 'quartile', color = 'white')
pyplot.show()
seaborn.violinplot(data = meta_data[ ['image_max'] ], inner = 'quartile', color = 'white')
pyplot.show()
...
all_the_text_features = load_features(text_data, 'data/text-features', text_input_size)
all_the_image_features = load_features(image_data, 'data/img-features', image_input_size)
...
train_callbacks = [
  tf.keras.callbacks.ModelCheckpoint(filepath = os.path.join(checkpoint_dir, checkpoint_dir), verbose = 1, save_freq='epoch'),
...
all_the_image_features = load_features(image_features_data, 'data/img-features', image_input_size)
all_the_text_features = load_features(text_features_data, 'data/text-features', text_input_size)
...
for i in range(nb_items):
  probas[i] = meme_classifier(tf.expand_dims(all_the_inputs[i, :], 0), training = False)
